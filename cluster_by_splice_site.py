...
def get_op_length(cigartuples):
    ...
    op_dict = collections.Counter({'M':0, 'I':1, 'D':2, 'N':3, 'S':4, 'H':5, 'P':6, '=':7, 'X':8, 'B':9})
    for op in op_dict:
...
def clus_by_splice_site(in_bam):
    ...
    filter_bam_fp = pysam.AlignmentFile(in_bam, 'rb')
    single_exon_set = []
    splice_site_set = []
    iter = filter_bam_fp.fetch()
    for record in iter:
        ...
        if len(N_idx) > 0: # multi-exon read
            ...
            splice_site_set.append((query_name, record.reference_name, record.is_reverse, first_site, last_site, query_seq))
        else: # single-exon read
            single_exon_set.append((query_name, record.reference_name, record.is_reverse, ref_start, ref_end, query_seq))
