...
def filter_bam(argv):
    ...
    fb.filter_bam(samtools, gmap_sam, gmap_filter_sort_bam, gmap_thread, align_rate, match_rate, best_rate, rRNA)


def gen_cluster(argv):
    gmap_filter_sort_bam = argv['gmap_filter_sort_bam']
    cbss.clus_by_splice_site(gmap_filter_sort_bam)
...
if __name__ == '__main__':
    ...
    bam_filter_group.add_argument('-a', '--align_rate', default=0.8, type=float,
                                  help='minimum fraction of aligned bases [0.80]')
    argument = bam_filter_group.add_argument('-m', '--match_rate', default=0.85, type=float,
                                             help='minimum fraction of identically matched bases [0.85]')
    bam_filter_group.add_argument('-b', '--best_rate', default=0.8, type=float,
    ...
    gmap_group.add_argument('-g', '--gmap_bin', type=str, default='gmap', help='path of GMAP [gmap]')
    gmap_group.add_argument('-D', '--gmap_genome_dir', type=str, help='GMAP genome directory')
    gmap_group.add_argument('-d', '--gmap_db', type=str, required=True, help='GMAP reference genome database')
    ...
    poa_group.add_argument('-c', '--poa_cpus', type=int, default=1,
                           help='number of processors used to run POA at one time [1]')
